        ...
        for j in range(len(rando[i])):
            for k in range(len(rando[i][j])):    
                if len(trueStatements) > i and len(trueStatements[i]) > j and len(trueStatements[i][j]) > k: #FOR VERY TRUE STATEMENT
                    ...
                    levTR = levTR + findBestMatch(trueStatements[i][j][k],flatRand[i])                         #compare to best match in random                  
                    if (len(newStatements) > i and len(newStatements[i]) > 0):
                ...
                if (len(trueStatements) > i and len(trueStatements[i]) > 0):
                    countRan = countRan + 1
                    levRT = levRT + findBestMatch(rando[i][j][k],flatTrue[i])                         #if there is a true KB corresponding to this random data, compare the random statement to its best match in the true statements
...
def distanceEvaluations(log,shape,newPreds,truePreds,newStatements,trueStatements,conceptSpace,roleSpace,syn):
    ...
    return [[levTR,levRT,levTN,levNT,sizeTrue,sizeNew,sizeRan],[levTR2,levRT2,levTN2,levNT2,sizeTrue2,sizeNew2,sizeRan2],[custTR,custRT,custTN,custNT,countTrue,countNew,countRan]]
...
def runNthTime(log,epochs,learningRate,conceptSpace,roleSpace,nthData,syn):
    ...
    shallowSystem(int(epochs/2),learningRate,log,conceptSpace,roleSpace,nthData,syn)    
    
    tf.reset_default_graph()
    
    deepSystem(epochs,learningRate/2,log,conceptSpace,roleSpace,nthData,syn)
...
def nTimesCrossValidate(n,epochs,learningRate,conceptSpace,roleSpace,syn,mix):
    if syn:
        ...
        if mix:
            ...
            labels = collapseLabelMap(localMaps,stats[0][2],stats[1][2],stats[4][1])
            allTheData = crossValidationSplitAllData(n,KBs,supports,outputs,sKBs,ssupports,soutputs,labels)
        else: allTheData = crossValidationSplitAllData(n,KBs,supports,outputs,None,None,None,None)
    else:
        if not os.path.isdir("snoutput"): os.mkdir("snoutput")
        ...
        if mix:
            ...
            allTheData = crossValidationSplitAllData(n,KBs,supports,outputs,sKBs,ssupports,soutputs,labels)
        else: allTheData =  crossValidationSplitAllData(n,KBs,supports,outputs,None,None,None,labels)
    
    for i in range(n):
        KB = allTheData[0][i]
        support = allTheData[1][i]
        output = allTheData[2][i]
        label = allTheData[3][i] if allTheData[3] != None else None
        sKB = allTheData[4][i] if allTheData[4] != None else None
        ssupport = allTheData[5][i] if allTheData[5] != None else None
        soutput = allTheData[6][i] if allTheData[6] != None else None
        runNthTime(open("fold[{}].txt".format(i),"w"),epochs,learningRate,conceptSpace,roleSpace,formatDataN(KB,support,output,label,sKB,ssupport,soutput),syn)
    pass

def crossValidationSplitAllData(n,KBs,supports,outputs,sKBs,ssupports,soutputs,localMaps):
    ...
    for i in range(len(indexes)):
        ...
        support = []
        output = []
        if crossLabelsTest != None: labels = []
        else: labels = None
        ...
        for j in range(len(indexes[i])):
            KB.append(KBs[indexes[i][j]])
            support.append(supports[indexes[i][j]])
            output.append(outputs[indexes[i][j]])
            if labels != None: labels.append(KBs[indexes[i][j]])
            if sKB != None:
                sKB.append(sKBs[indexes[i][j]])
                ssupport.append(ssupports[indexes[i][j]])
                soutput.append(soutputs[indexes[i][j]])                
        crossKBsTest.append(array(KB))
        crossSupportsTest.append(support)
        crossOutputsTest.append(output)
        if crossLabelsTest != None: crossLabels.append(labels)
        if scrossKBsTest != None:
            scrossKBs.append(sKB)
            scrossSupports.append(ssupport)
            scrossOutputs.append(soutput)
    
    crossKBsTest = array(crossKBsTest) 
    
    for i in range(len(crossOutputsTest)):
        crossOutputsTest[i] = array(crossOutputsTest[i])
    
    crossKBsTrain = []#numpy.empty((n,(len(KBs)-len(crossKBsTest[0]))),dtype=numpy.ndarray)
    crossOutputsTrain = []#numpy.empty((n,(len(KBs)-len(crossKBsTest[0]))),dtype=numpy.ndarray)
    crossSupportsTrain = []#numpy.empty((n,(len(KBs)-len(crossKBsTest[0]))),dtype=numpy.ndarray)
    ...
    for i in range(len(indexes)):
        ...
        for j in range(len(KBs)):
            if j not in indexes[i]:
                crossKBTrain.append(KBs[i])
                crossSupportTrain.append(supports[i])
                crossOutputTrain.append(outputs[i])
        crossKBsTrain.append(array(crossKBTrain))
        crossSupportTrain.append(array(crossSupportTrain))
        crossOutputTrain.append(array(crossOutputTrain))        
            
    crossKBsTrain = array(crossKBsTrain)
    crossOutputsTrain = array(crossOutputsTrain)
    crossSupportsTrain = array(crossSupportsTrain)
    ...
    junk = []
    for i in range(n):
        a,b = vecToStatements(y_test,conceptSpace,roleSpace)
        stuff.append(a)
        junk.append(b)
    
    return crossKBsTest,crossKBsTrain,crossSupportsTrain,crossSupportsTest,crossOutputsTrain,crossOutputsTest,stuff,junk
'''
fileShapes1 = [3,124,48]
